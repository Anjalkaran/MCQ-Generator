...
const getPerformanceTier = (average: number): { label: string; className: string } => {
  if (average >= 80) return { label: 'Excellent', className: 'bg-green-100 text-green-800 border-green-300' };
  if (average >= 60) return { label: 'Good', className: 'bg-blue-100 text-blue-800 border-blue-300' };
  if (average >= 40) return { label: 'Average', className: 'bg-yellow-100 text-yellow-800 border-yellow-300' };
  return { label: 'Needs Improvement', className: 'bg-red-100 text-red-800 border-red-300' };
};
