...
/**
 * @fileOverview API route to handle multi-file uploads for Daily Tests.
...
export async function POST(request: Request) {
    ...
    try {
        ...
        // 2. Create the Daily Test entry referencing the bank document
        const dailyTestRef = await db.collection('dailyTests').add({
            title,
            examCategories,
        ...
        return NextResponse.json({
            message: 'Daily test created successfully with merged questions.',
            newTest: {
                id: dailyTestRef.id,
                title,
                examCategories,
