...
function DailyTestCard({ test }: { test: DailyTest }) {
...
export default function DailyTestPage() {
    ...
    const filteredTests = dailyTests;

    return (
        <div className="space-y-6">
            <div className="space-y-2 text-center">
                <h1 className="text-3xl font-bold tracking-tight">Daily Tests</h1>
                <p className="text-muted-foreground">Short targeted practice tests released daily to keep you consistent.</p>
            </div>

            {filteredTests.length > 0 ? (
                <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                    {filteredTests.map(test => <DailyTestCard key={test.id} test={test} />)}
                </div>
            ) : (
                <div className="text-center py-20 border-2 border-dashed rounded-lg">
                    <Clock className="mx-auto h-12 w-12 text-muted-foreground mb-4 opacity-20" />
                    <p className="text-muted-foreground font-medium">No daily tests have been added for the {userData?.examCategory} category yet.</p>
                </div>
            )}
        </div>
    );
