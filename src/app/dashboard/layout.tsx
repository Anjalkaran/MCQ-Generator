...
export default function DashboardLayout({
  ...
  useEffect(() => {
    ...
    if (user?.uid) {
      ...
      sendHeartbeat();
      intervalId = setInterval(sendHeartbeat, 600000); // every 10 minutes
    }
    return () => {
      if (intervalId) {
  ...
  useEffect(() => {
    ...
    if (userData?.email && ADMIN_EMAILS.includes(userData.email)) {
        ...
        intervalId = setInterval(fetchAndSetOnlineUsers, 300000); // Refresh every 5 minutes
    }
    return () => {
        if (intervalId) {
            clearInterval(intervalId);
  ...
  useEffect(() => {
    if (userData?.email && ADMIN_EMAILS.includes(userData.email)) {
        getDashboardData(userData.uid).then(data => {
