  ...
  async input => {
    ...
    for (const part of blueprint.parts) {
      for (const section of part.sections) {
        ...
        // Handle General Knowledge, which is generated by AI
        ...
        // Handle random selection from a pool of topics
            ...
            const allMcqDocsForSection = await Promise.all(relevantTopicIds.map(id => getTopicMCQs(id)));
            ...
            allMcqDocsForSection.flat().forEach(doc => {
                 try {
                    ...
                    if (parsed.mcqs && Array.isArray(parsed.mcqs)) {
                        canonicalQuestions.push(...parsed.mcqs.map((mcq: any) => ({ ...mcq, sourceDocId: doc.id })));
                    }
                } catch(e) {/* ignore */}
            });
