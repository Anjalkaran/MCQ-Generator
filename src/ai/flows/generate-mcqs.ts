...
const generateMCQsFlow = ai.defineFlow(
  ...
  async (input) => {
    ...
    // Check if the topic is General Knowledge/Awareness or Arithmetic, which should be generated by AI
    const knowledgeBasedCategories = ["General Awareness", "Basic Arithmetics"];
    if (input.category && knowledgeBasedCategories.includes(input.category)) {
